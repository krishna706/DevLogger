...
-(void)optionsTapped
{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Documents Log" style:UIAlertActionStyleDefault handler:^(UIAlertAction*action)
    {

        [alertController dismissViewControllerAnimated:YES completion:nil];
      //  RapidDocumentController *rapidDocumentController = [[RapidDocumentController alloc] init ];
       // [self.navigationController pushViewController:rapidDocumentController animated:YES];
...
-(void)stopRecording
{
    ...
    if ([ASScreenRecorder sharedInstance].isRecording)
    {
        ...
        [[ASScreenRecorder sharedInstance] stopRecordingWithCompletion:^{
            ...
            [[[UIAlertView alloc] initWithTitle:@"Recording" message:@"Screen Video is saved in gallery!!!!" delegate:nil cancelButtonTitle:@"Okay" otherButtonTitles:nil] show];
            //[self playEndSound];
